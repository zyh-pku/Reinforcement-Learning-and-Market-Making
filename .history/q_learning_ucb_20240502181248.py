...
class QLearningAgent:
    ...
    def __init__(self, env, dim_midprice_grid, dim_inventory_grid, dim_action_ask_price, dim_action_buy_price, Delta=0.1,  \
        ...
        self.V_error_track_full = np.zeros( (self.dim_midprice_grid, self.dim_inventory_grid, N_RL_iter) )
    ...
    def plot_V_error_result(self,):
        ...
        plt.xlabel('step')
        plt.ylabel('value error supremum norm')
        plt.show()    

        fig, axs = plt.subplots(self.dim_midprice_grid, self.dim_inventory_grid, figsize=(9, 9)) # the size for slides
        global_min = np.min(self.V_error_track_full)
        global_max = np.max(self.V_error_track_full)
        for idx_midprice in range(self.dim_midprice_grid):
            for idx_inventory in range(self.dim_inventory_grid):
                # Extracting data for each subplot
                subplot_data = self.V_error_track_full[idx_midprice, idx_inventory, :M]
    ...
    def plot_result(self,):
        ...
        M=self.N_RL_iter
        plt.plot(self.Q_table_track[0,2,1,3,:M], label='ask=1')
        plt.plot(self.Q_table_track[0,2,2,3,:M], label='ask=2')
        plt.plot(self.Q_table_track[0,2,0,3,:M], label='ask=0')
        plt.plot(self.Q_table_track[0,2,3,3,:M], label='ask=3')
        plt.legend()
        plt.xlabel('step')
        plt.ylabel('Q(s,a)')
        plt.show()
